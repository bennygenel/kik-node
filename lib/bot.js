...
class Bot {
    ...
    incoming() {
        ...
        return (req, res, next) => {
            ...
            console.log('bot reqUrl: ', reqUrl, 'incomingPath: ', this.incomingPath, 'equal: ', (reqUrl === this.incomingPath));
            if (reqUrl.indexOf(this.scanCodePath) === 0) {
                ...
                // req.on('end', () => {
                    ...
                    try {
                        if(typeof body === 'object') parsed = body;
                        else parsed = JSON.parse(body);
                    }
                    catch (ex) {
                        console.log('try error: ', JSON.stringify(ex));
                        res.statusCode = 400;
